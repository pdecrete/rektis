...
//                    'print' => ['POST'],
    ...
    protected function generatePrintDocument($leaveModel)
    {
        ...
        for ($c = 0; $c < $all_count; $c++) {
            ...
            $rem = $currentModel->getmydaysLeft($currentModel->employee, $currentModel->type, date("Y", strtotime($currentModel->start_date)), $currentModel->end_date);
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->typeObj->check == true) {
                ...
                if (($prev_days = (int)$model->getOldAttribute('duration')) != (int)$model->duration) {
                    ...
                    if ($left + $prev_days - $model->duration < 0) {
                        $str = 'Το υπόλοιπο των αδειών του εργαζομένου δεν επαρκεί για την καταχώριση αυτής της άδειας. Παρακαλώ διορθώστε. ';
                        Yii::$app->session->setFlash('danger', $str);
