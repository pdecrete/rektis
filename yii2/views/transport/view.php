            ...
            DetailView::widget([
                ...
                'attributes' => [
                    ...
                    [
                        ...
                        'value' => $model->transportFund1 === null ? null : Yii::t('app', '{kae} ({name} - {date}) - {service_n}', [
                            'kae' => $model->transportFund1->kae,
                            'name' => $model->transportFund1->name,
                            'date' => \Yii::$app->formatter->asDate($model->transportFund1->date),
                            'service_n' => $model->transportFund1->service0->name,
                        ]),
                        'format' => 'raw'
                    ],
                    [
                        'label' => $model->getAttributeLabel('fund2'),
                        'value' => $model->transportFund2 === null ? null : Yii::t('app', '{kae} ({name} - {date}) - {service_n}', [
                            'kae' => $model->transportFund2->kae,
                            'name' => $model->transportFund2->name,
                            'date' => \Yii::$app->formatter->asDate($model->transportFund2->date),
                            'service_n' => $model->transportFund2->service0->name,
                        ]),
                        'format' => 'raw'
                    ],
                    [
                        'label' => $model->getAttributeLabel('fund3'),
                        'value' => $model->transportFund3 === null ? null : Yii::t('app', '{kae} ({name} - {date}) - {service_n}', [
                                'kae' => $model->transportFund3->kae,
                                'name' => $model->transportFund3->name,
                                'date' => \Yii::$app->formatter->asDate($model->transportFund3->date),
                                'service_n' => $model->transportFund3->service0->name,
                            ]),
                        'format' => 'raw'
                    ],
                    'decision_protocol',
