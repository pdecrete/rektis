...
$inline_td_css = 'style="text-align: right;border: 1px solid black;"';
$sum_net_value = 0;
$sum_vat = 0;
$sum_taxes = 0;
...
<div class="finance-expenditure-index">
	...
	<table style="width:100%;border-collapse: collapse;">
		...
		<?php   foreach($models as $model): 
  		...
  		?>				
            		<tr>
            			<td <?= $inline_td_css?>><?= $model['SUPPLIER']['suppl_name']; ?></td>
            			<td <?= $inline_td_css?>><?= $model['INVOICE']['inv_number']; ?></td>
            			<td <?= $inline_td_css?>><?= $model['EXPENDITURE']['exp_description']; ?></td>
            			<td <?= $inline_td_css?>><?= number_format($net_value, 2, ',', '.') ?></td>
            			<td <?= $inline_td_css?>><?= number_format($vat, 2, ',', '.') ?></td>
            			<td <?= $inline_td_css?>><?= number_format($net_value + $vat, 2, ',', '.') ?></td>
            			<td <?= $inline_td_css?>></td>
            			<td <?= $inline_td_css?>><?= number_format($payable_amount, 2, ',', '.') ?></td>
            		</tr>
        <?php
		...
		<tr>
			...
			<td <?= $inline_td_css?>></td>
			<td <?= $inline_td_css?>></td>
			<td <?= $inline_td_css?>><?= number_format($sum_net_value, 2, ',', '.') ?></td>							
			<td <?= $inline_td_css?>><?= number_format($sum_vat, 2, ',', '.') ?></td>
			<td <?= $inline_td_css?>><?= number_format($sum_net_value + $sum_vat, 2, ',', '.') ?></td>
			<td <?= $inline_td_css?>><?= number_format($sum_taxes, 2, ',', '.') ?></td>
			<td <?= $inline_td_css?>><?= number_format($sum_payable_amount, 2, ',', '.') ?></td>
		</tr>						
	</table>
	<br /><br />
